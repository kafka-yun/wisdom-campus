<template>
  <div class="camera_outer">
    ...
    <div class="container">
      ...
      <el-button @click="setImage()">录入</el-button>
    </div>
  </div>
</template>
  <script>
...
export default {
  ...
  methods: {
    ...
    login(email,image){
      var Xheader= JSON.parse(localStorage.getItem("X-Header"))
      instance({
        method:"post",
        url:"/api/user/face/registered",
        data:{
          "image":image,
          "email":email
        },
        headers:{
          "X-Token":Xheader.token,
          "X-Key":Xheader.key,
          "X-BusinessId":Xheader.BusniessId
        }
      }).then(res=>{
